...
${books[indexBook]}</svg></p></div></div><div class="book_description_container"><div class="book_descriptions"><p>Author</p><p>Erscheinungsjahr</p><p>Genre</p></div><div class="book_desicription_values"><p>: ${books[indexBook].author}</p><p>: ${books[indexBook].publishedYear}</p><p>: ${books[indexBook].genre}</p></div></div><div class="comment_section"><h3>Kommentare :</h3><table id="comment_window${indexBook}"><tbody></tbody></table></div></div>
`
}

function getCommentTemplate(indexBook, indexComment){
  return `<tr class = "comment" id = "comment${indexComment}"><td><p id = "comment_name${indexComment}">${books[indexBook].comments[indexComment].name}</p></td><td><p id = "comment_text${indexComment}">${books[indexBook].comments[indexComment].comment}</td></tr>`}